...
class DataProcessor:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_features["balance_change_org"] = df_features["newbalanceOrig"] - df_features["oldbalanceOrg"]
        ...
        df_features["amount_to_balance_ratio"] = df_features["amount"] / (df_features["oldbalanceOrg"] + 1)  # Add 1 to avoid division by zero
        
        # Flag for zero balance after transaction
        df_features["zero_balance_after"] = (df_features["newbalanceOrig"] == 0).astype(int)
        
        # Flag for large balance changes
        balance_change_threshold = df_features["balance_change_org"].quantile(0.95)
        df_features["large_balance_change"] = (df_features["balance_change_org"].abs() > balance_change_threshold).astype(int)
        
        # Transaction type risk (based on fraud rates in the data)
        type_risk = {
            "PAYMENT": 0.1,
            "TRANSFER": 0.5,
            "CASH_OUT": 0.4,
        ...
        df_features["type_risk"] = df_features["type"].map(type_risk)
        ...
        # Flag for unusual transaction amounts
    ...
    def prepare_train_test_data(
        ...
        # Adjust val_size to be relative to the size of X_temp
